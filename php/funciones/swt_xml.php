<?php 
	...
	$consulta = "SELECT * FROM leagues WHERE id = '$id_liga'";  
	...
	if(!empty($html)){
		...
		if ($id_wihi_liga_ori == "Por confirmar") {
			$consulta = "UPDATE league SET name_uk = '$liga_nombre' WHERE id = '$id_liga'";
						...
						foreach($pokemon_row as $row){
							...
							if ($frase != " ") {
								...
								if (preg_match("/Tuesday/", $frase) OR preg_match("/Wednesday/", $frase) OR preg_match("/Thursday/", $frase) OR preg_match("/Friday/", $frase) OR preg_match("/Sunday/", $frase) OR preg_match("/Saturday/", $frase) OR preg_match("/Monday/", $frase)) {
									...
									echo "MES: $diadf $mesdf $anyodf <br><br>";		
									...
									$url_p_futbol = utf8_decode($url_p_futbol);				
									...
									for ($i=0; $i < 2; $i++) { 
										...
										$equipo1 = utf8_decode($division5[$i]);


										if ($pos_rec === false) {
										    $con_part1 = "SELECT * FROM teams WHERE name_uk = '".$equipo1."'";
											$ex_con_part1 = $conexion->query($con_part1);
											$regs_con_part1 = $ex_con_part1->num_rows;

											if ($regs_con_part1 == '0') {
												$con_part2 = "INSERT INTO teams (name_uk, name) VALUES ('$equipo1','$equipo1')";
												...
												if($ex_con_part2){ 													
													...
													$con_eq_li1 = "SELECT * FROM league_team WHERE team_id = $id_equipo AND league_id = $id_liga";
													$ex_con_eq_li1 = $conexion->query($con_eq_li1);
													$regs_con_eq_li1 = $ex_con_eq_li1->num_rows;

													if ($regs_con_eq_li1 == 0) {
														$con_eq_li2 = "INSERT INTO league_team (team_id, league_id) VALUES ('$id_equipo','$id_liga')";
									...
									$con_market1 = "SELECT * FROM games WHERE url = '$url_p_futbol'";
									...
									if ($regs_con_market1 == '0') {
					            		...
					            		$id_p_futbol = $bus_con_market1["id"];
					            		$id_wihi_p = $bus_con_market1["id_web"]; 

					            		$hora_existe = $bus_con_market1["start"];
					            		...
					            		if ($id_categoria == '22' AND $hora_existe > $hoy) {
					            			...
					            			if ($hora_existe != $date_time) {
										 		$consulta_h2 = "UPDATE games SET start='$date_time' WHERE id='$id_p_futbol'";
					            	...
					            	if ($id_categoria == '21' OR $id_categoria == '28') {
					            		...
					            		$codigo_part[1] = "1!".$id_p_futbol."!".$id_ta;
					            	...
					            	for ($i=0; $i < count($codigo_part); $i++) { 
	        					       	...
	        					       	$cons = "SELECT * FROM competitors WHERE code = '$codigo'"; 
	        					       	...
	        					       	if ($nro_regs == '0') {
											...
											if($ex_cons2){
	        					       		...
	        					       		$id_participante_rs = $resultado['id'];

	        					       		if ($resultado['odd'] != $dividendo) {

	        					       			$consulta_34 = "UPDATE competitors SET odd = '$dividendo' WHERE id = '$id_participante_rs'";
